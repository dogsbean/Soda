...
//        }
    ...
    public void setMessagesToggled(boolean messagesToggled) {
        this.messagesToggled = messagesToggled;
        if(messagesToggled) {
            getPlayer().setMetadata("messagesToggled", new FixedMetadataValue(Soda.getInstance(), true));
            return;
        }

        getPlayer().removeMetadata("messagesToggled", Soda.getInstance());
    }
    ...
    public Tag getActiveTag() {
        return Soda.getInstance().getModuleHandler().getModule(TagHandler.class).getTag(this.tag.toUpperCase());
    }
