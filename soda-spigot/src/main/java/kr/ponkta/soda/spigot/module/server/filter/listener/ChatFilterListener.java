package kr.ponkta.soda.spigot.module.server.filter.listener;
...
public class ChatFilterListener extends ListenerModule {
    ...
    public void onPlayerChatLowest(AsyncPlayerChatEvent event) {
        if(event.getPlayer().hasPermission("Soda.filter.bypass")) return;
    ...
    public void onQuit(PlayerQuitEvent event) {
        ...
        if(player.hasMetadata("frozen"))
            Bukkit.getOnlinePlayers()
                    .stream()
                    .filter(staff -> staff.hasPermission("Soda.staff"))
