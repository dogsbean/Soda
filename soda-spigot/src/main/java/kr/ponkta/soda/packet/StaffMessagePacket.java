        ...
        profileHandler.load(uuid).thenAccept(profile -> {
            switch(type) {
                ...
                case ADMIN_CHAT: {
                    ...
                    profileHandler.getStaffProfiles().forEach(staffProfile -> {
                        Player player = Bukkit.getPlayer(staffProfile.getUuid());
                        if(player != null && player.isOnline() && !player.hasMetadata("togglestaff") && player.hasPermission("Soda.command.adminchat")) {
                            player.sendMessage(message);
                        }
                    });
                    break;
                ...
                case MANAGER_CHAT: {
                    ...
                    profileHandler.getStaffProfiles().forEach(staffProfile -> {
                        Player player = Bukkit.getPlayer(staffProfile.getUuid());
                        if(player != null && player.isOnline() && !player.hasMetadata("togglestaff") && player.hasPermission("Soda.command.managerchat")) {
                            player.sendMessage(message);
                        }
                    });
                    break;
